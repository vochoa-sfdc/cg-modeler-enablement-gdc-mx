...
if(!signatureBlobsAreEmpty) {
  promise = BoFactory.createListAsync("LoSysSignatureAttribute", {}).then(
    ...
    function (loSignatureBlob) {
      ...
      var tourReferencepkey = "";
      if(Utils.isDefined(ApplicationContext.get(BLConstants.APPCTX.DSD.CURRENT_TOUR)) && Utils.isDefined(ApplicationContext.get(BLConstants.APPCTX.DSD.CURRENT_TOUR).getPKey())) {
        tourReferencepkey = ApplicationContext.get(BLConstants.APPCTX.DSD.CURRENT_TOUR).getPKey();
      }
      ...
      for(var i = 1; i <= stepItems.length; i++) {
        ...
        var liSysSignatureBlob = {
          ...
          "tourReferenceKey" : tourReferencepkey,
