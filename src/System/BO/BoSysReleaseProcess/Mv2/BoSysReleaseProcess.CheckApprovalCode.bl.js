  ...
  if (me.get("signatureApprovalSuccessful" + stepNumber) !== "1") {
    ...
    if (stepItems.length > stepNumber - 1) {
      ...
      if (Utils.isEmptyString(id) || Utils.isEmptyString(code)) {
        ...
        promise = MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Validation"), Localization.resolve("ReleaseProcess_ApprovalCodeMissing"), buttonValues);
      } 
      else {
        ...
        promise = BoFactory.loadObjectByParamsAsync("LuUserApprovalCode", jsonQuery)
          ...
          if (!Utils.isDefined(lookup.getPKey()) || Utils.isEmptyString(lookup.getPKey())) {
            var buttonValues = {};
            buttonValues[Localization.resolve("OK")] = "ok";
            return MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Validation"), Localization.resolve("ReleaseProcess_ApprovalCodeMismatch"), buttonValues);
          } else {
