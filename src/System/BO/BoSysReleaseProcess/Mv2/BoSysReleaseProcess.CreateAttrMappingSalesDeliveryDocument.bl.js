...
let attributes = Utils.createDictionary();
let ref = this.getReferenceObject();
let tour = ApplicationContext.get(BLConstants.APPCTX.DSD.CURRENT_TOUR);
...
if (ref.getLuDeliveryRecipient) {  
  ...
  customerName = ref.getLuDeliveryRecipient().getName();
}
...
if((!Utils.isEmptyString(customerName))) {
  attributes.add("Customer", customerName);
}
