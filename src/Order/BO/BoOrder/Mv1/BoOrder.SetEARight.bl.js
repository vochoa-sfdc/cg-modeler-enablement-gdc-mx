 ...
 * @returns promise
 * @description This function controls the editability of the order BO and its related objects.
 */
...
//  ###                                                                                              ###
//  ###   1.) Phase in ('Released' 'Closed', 'Canceled', 'Ready', 'Feedback', 'Voided')              ###
//  ###   2.) The upload of the order to the backend has failed: Order._syncStatus = 'NOT_SYNCABLE'  ###
//  ###   3.) The order is not mobility relevant: Order_Template.Mobility_Relevant = 'No'            ###
//  ###   4.) The user is not the responsible of the order: Current user <> Order.Responsible        ###
// #####################################################################################################  
if (
  (phase === BLConstants.Order.PHASE_CLOSED) ||
  (phase === BLConstants.Order.PHASE_CANCELED) ||
  (phase === BLConstants.Order.PHASE_READY) ||
  (phase === BLConstants.Order.PHASE_FEEDBACK) ||
  (phase === BLConstants.Order.PHASE_VOIDED) ||
  ...
  if (me.getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_ORDER_ENTRY || me.getDocumentType() === "Invoice") {
    acl.removeRight(AclObjectType.PROPERTY, "paymentReason", AclPermission.VISIBLE);
  }
  else {
...
if(
    phase === BLConstants.Order.PHASE_RELEASED || 
    me.getSyncStatus() === BLConstants.Order.NOT_SYNCABLE
  ) {
  acl.removeRight(AclObjectType.OBJECT, "BoOrder", AclPermission.EDIT);
...
if(me.getIsOrderPaymentRelevant() === '0') {
  ...
  if (
    me.getDocTaType() === "CashOrder" ||
    me.getDocTaType() === "InvoiceCashInvoice" ||
    me.getDocTaType() === "VanSalesCashInvoice" ||
    me.getDocTaType() === "CashInvoice"
  ) {
...
var displayQtySuggestionInfo =
(
  (
    //Quantitiy suggestion check for non-selectable promotion scenario
    (((me.getBoOrderMeta().getConsiderQuantitySuggestion() === "LQty" && me.getBoOrderMeta().getLoOrderItemMetas().getMainItemTemplate().getQuantitySuggestionPolicy() === "OrderedQuantity") ||
      me.getBoOrderMeta().getConsiderQuantitySuggestion() === "UserExit") && 
      (!Utils.isDefined(me.getSelectedPromotionPKey()) || Utils.isEmptyString(me.getSelectedPromotionPKey()))) ||
    //special order check for non-selectable promotion scenario
    (!Utils.isEmptyString(me.getSelectedPromotionPKey()) && me.getLoSelectablePromotion().getCurrent().getSpecialPromoted() === '1')
  ) &&
  phase === BLConstants.Order.PHASE_INITIAL
);
...
//###  Use Case- Add Item to Item List Automatically:                            ###
...
if (Utils.isDefined(me.getBoOrderMeta())) {
  ...
  if(me.getBoOrderMeta().getSdoSubType() != BLConstants.Order.DOCUMENT_TYPE_DELIVERY) {
    ...
    acl.removeRight(AclObjectType.PROPERTY, "customerOrderId", AclPermission.EDIT);
    acl.addRight(AclObjectType.PROPERTY, "asn", AclPermission.VISIBLE);
    acl.addRight(AclObjectType.PROPERTY, "asn", AclPermission.EDIT);
    me.getLuDeliveryRecipient().getACL().removeRight(AclObjectType.OBJECT, "luDeliveryRecipient", AclPermission.EDIT);
    if(me.getBoOrderMeta().getAllowHeaderDiscount() == 1){
      acl.addRight(AclObjectType.PROPERTY, "headerDiscount", AclPermission.EDIT);
    }
    if(me.getBoOrderMeta().getCanPurchaseNumberUpdateAllowed() == 1){
      acl.addRight(AclObjectType.PROPERTY, "customerOrderId", AclPermission.EDIT);
    }
  }

  //invoiceId for Van-Sales#, Cash- and Credit documents
  if (
    (me.getBoOrderMeta().getSdoSubType() == BLConstants.Order.DOCUMENT_TYPE_ORDER_ENTRY &&
      (me.getBoOrderMeta().getDocTaType() == "VanSalesCashInvoice" ||
        me.getBoOrderMeta().getDocTaType() == "VanSalesNoPaymentInvoice")) ||
    (me.getBoOrderMeta().getSdoSubType() == BLConstants.Order.DOCUMENT_TYPE_DELIVERY &&
      (me.getBoOrderMeta().getDocTaType() == "CashInvoice" ||
        me.getBoOrderMeta().getDocTaType() == "CreditInvoice"))
  ) {
...
// #######################################################
//  ###   Rewards and Hurdles / Selectable Promotions  ###
// #######################################################
...
// ###############
//#############################################################################
// ###   Check related visit status for DSD-Orders                          ###
// ###   --> Lock order if linked to a visit which is not editable          ###
//#############################################################################
var isDSDMode = ApplicationContext.get(BLConstants.APPCTX.DSD.DSDMODE);
if (!Utils.isEmptyString(me.getCallInContext_clbMainPKey()) && isDSDMode){
  promise =  BoFactory.loadObjectByParamsAsync(BO_CALL, {"params": [
    {field: "pKey", value: me.getCallInContext_clbMainPKey()},
    {field: "referenceUserPKey", value: ApplicationContext.get('user').getPKey()},
  ]})
    .then(function (boCall) {

      //lock order if related visit is read only
      if (Utils.isDefined(boCall) && boCall.isReadOnly(isDSDMode)) {
