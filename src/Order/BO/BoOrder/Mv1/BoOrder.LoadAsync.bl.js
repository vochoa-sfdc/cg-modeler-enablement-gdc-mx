 ...
 * @returns promise
 * @description  Asynchronously loads an order with all its related data including items, payments, promotions, and metadata.
 *  Handles different loading scenarios (regular, cancellation, deletion) and performs necessary initializations such as pricing calculations, inventory checks, and order validation. 
 *  Sets up order splitting, payment methods and promotion handling based on order configuration.
 *  For editable orders, validates items and calculates pricing using either complex or simple pricing engine.
 */
  ...
  promise = Facade.getObjectAsync(BO_ORDER, jsonQuery).then(
    ...
    function (boOrderMeta) {
      if (Utils.isDefined(boOrderMeta.getLoOrderItemMetas())) {
        ...
        // Load Order Items from temp table
        // To fill and load from temp table IvcRefPKeys(inventory search keys) are already needed to find correct inventory for inventory filtering
        return me.loadOrderItems().then(
          ...
          function () {
            ...
            //Selectable Promotion - Load Promotions and initialize HurdleEvaluationHelper
            if (me.getBoOrderMeta().getConsiderSelectablePromotion() == "1") {
              return BoFactory.loadObjectByParamsAsync(LO_SELECTABLEPROMOTION, {
                currentDate: Utils.createAnsiDateToday(),
                customer: me.getOrdererPKey(),
                specialOrderHandling: me.getBoOrderMeta().getSpecialOrderHandling()
              }).then(
                function (selectablePromotions) {
                  me.setLoSelectablePromotion(selectablePromotions);
                  return BoFactory.createObjectAsync(BO_HURDLEEVALUATIONHELPER, {selectablePromotions: selectablePromotions, orderCache: me});
                }).then(
          ...
          function () {
            ...
            //Order Splitting - determine rules and groups
          ...
          function () {
            ...
            //Release Process - determine release process
          ...
          function (sysReleaseProcessPKey) {
            ...
            //Release Process - load relesae process steps
            if (!Utils.isEmptyString(sysReleaseProcessPKey)) {
              return BoFactory.loadListAsync(
                LO_SYSRELEASEPROCESSSTEP,
                {
                  params: [
                    {
                      field: 'sysReleaseProcessPKey',
                      operator: 'EQ',
                      value: sysReleaseProcessPKey
                    }
                  ]
                }
              );
          ...
          function (loSysReleaseProcessStep) {
            ...
            //Release Process - set release process step flag
          ...
          .then(function (LoSysSignatureBlob){
            ...
            if (Utils.isDefined(LoSysSignatureBlob))  {
              ...
              //Signature - load Signature Attributes
              return Facade.getListAsync(LO_SYSSIGNATUREATTRIBUTE,jsonQueryForSignature);
            }
          })
          .then(function (LoSysSignatureAttribute){

            //Signature - set signature attributes
            if (Utils.isDefined(LoSysSignatureAttribute))  {
              me.setLoSysSignatureAttribute(BoFactory.instantiate(LO_SYSSIGNATUREATTRIBUTE));
            ...
            //Order Splitting - Execute Splitting Groups  
            return BoFactory.createListAsync(LO_SPLITTINGGROUPRESULT, {});
          }).then(
          function (list) {
          ...
          function (loNotes) {
            ...
            //Selectable Promotion - determine and set promotion count
            me.setPromotionCount(0);
            if (me.boOrderMeta.considerSelectablePromotion == "1") {
              var jsonParams = [];
              var jsonQuery = {};
              ...
              return Facade.getObjectAsync(LU_PROMOTIONCOUNT, jsonQuery).then(
          ...
          function () {
            ...
            //Payments - load payments if payment relevant
          ...
          function () {
            ...
            //if editable check if calculation is outdated
            if (!(checkIfPhaseConditionIsMet ||
                  me.getBoOrderMeta().getMobilityRelevant() == "0" ||
                  me.getResponsiblePKey() != ApplicationContext.get("user").getPKey()
                 )) {
            ...
            if (
                ...
                function () {
                  ...
                  return addProductsPromise.then(
                    function () {
                      ...
                      if (!checkIfPhaseConditionIsMet && me.getSyncStatus() !== BLConstants.Order.NOT_SYNCABLE) {
                       ...
                       /*during order load, if any product is found to be invalid then its quantity is set to 0
                        ...
                        //Invalidation check - Check if there exist invalid item in order
                        ...
                        //do not invalidate item if order is no longer editable
                    ...
                    function () {
                      ...
                      //Selectable Promotion - If editable then restore reward selection and reevaluate hurdles
                      if ((me.getBoOrderMeta().getConsiderSelectablePromotion() == "1" && 
                    ...
                    function () {
                      ...
                      if (isOrderValidForCalculation && isEditMode && (isCalculationMandatory || isCalculationRequired) && me.getSyncStatus() !== BLConstants.Order.NOT_SYNCABLE) {
                        ...
                        //PPP - calculate order and handle payment
                        if(me.getIsOrderPaymentRelevant() === '1'){
                          ...
                          //PPP - Only calculate order if not payment relevant
                          return me.cpCalculate().then(
                            function () {
                              return when.resolve(me);
                            });
              ...
              return BoFactory.createObjectAsync("BoHelperSimplePricingCalculator", {}).then(
                ...
                function () {
                  ...
                  //Selectable Promotion - If editable then restore reward selection and reevaluate hurdle
                  me.setIsNotReadyForHurdleEvaluation("0");
                  if(me.getBoOrderMeta().getConsiderSelectablePromotion() == "1" && 
                     ![BLConstants.Order.PHASE_RELEASED, BLConstants.Order.PHASE_READY].includes(me.getPhase()) && me.getSyncStatus() !== BLConstants.Order.NOT_SYNCABLE) {
        ...
        //set invalid if nor order template is defined
