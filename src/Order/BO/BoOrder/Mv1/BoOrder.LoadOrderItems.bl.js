 ...
 * @async
 * @namespace CORE
 * @description Handles the loading of order items by merging them into a temporary table, applying filtering conditions, 
 * and retrieving relevant products. It also considers quantity suggestions and create the display information accordingly.
 * @returns promise
 */
function loadOrderItems(){
    var me = this;
...
//#######################################################
//### Temp table handling (TmpOrderItemMergeResult_T) ###
//### Fill Temp Table and read from Temp Table        ###
//#######################################################
var promise = BoFactory.createListAsync("LoOrderItems", {}).then(
  function (loItems) {
    ...
    loOrderItems = loItems;
      ...
      //filling the temp table (orderItemsQuery is calculated in getJsonQueryForLoItems)
      return loOrderItems.mergeOrderItemList(orderItemsQuery).then(
        function () {

          //get params to load from temp table
          jsonQuery.params= loOrderItems.buildQueryCondition(orderItemsQuery, "ALL");

          //reading relevant products from temp table
