  ...
  for (var i = 0; i < currentEvent.modified.length; i++) {
    ...
    switch (currentEvent.modified[i]) {
      ...
      case "pricingInfo10":
        ...
        // if the item is already available or added to the pricing engine the pricingInfo update
        // must be handed over to the pricing engine so that the next calculation (which relies on pricingInfo fields) is correct
        // Note: switching the quantity to a value > 0 does not necessarily mean that pricingInfo fields will also change. 
        // PricingInfo fields are linked with hurdles and rewards!
        if (
            mainItem.getQuantity() > 0 || 
            (CP.PricingHandler.getInstance().hasOrderItem(mainItem.getPKey()))
          ) {
          orderUnitItem = me.getLoItems().getFirstOrderUnitItemForMainItem(mainItem);
          pricingTriggerObject = {
            mainItem: mainItem,
...
if (isUpdateRequired) {
  ...
  var qtyChangedHandler = function(x) {
    ...
    var documentType = me.getBoOrderMeta().getSdoSubType();
    var isDocumentTypeDelivery = documentType === BLConstants.Order.DOCUMENT_TYPE_DELIVERY;

    if (isDocumentTypeDelivery || ((me.getBoOrderMeta().getConsiderQuantitySuggestion() === "PQty" || isFreeItem || mainItem.getSpecialPromoted() =="1") &&
        (mainItem.getMergeEngine_invalidated() === "0") && mainItem.getEdited() !== "0")) {
      var modifyPresetQuantityOption = " ";
      if (isDocumentTypeDelivery && Utils.isDefined(sdoItemMeta)) {
        modifyPresetQuantityOption = sdoItemMeta.getModifyPresetQuantity();
      } else if (!Utils.isEmptyString(me.getLuDeliveryRecipient().getPKey()) && !Utils.isEmptyString(me.getLuDeliveryRecipient().getSdoModifyPresetQuantity())) {
      ...
      var setPresetQuantity = function(item, modifyPresetQtyOption, presetValue){
        // We must ensure to not trigger ChangedHandler again, detaching events
        me.getLoItems().suspendListRefresh();
        if (modifyPresetQtyOption === "No") {
          item.setQuantity(presetValue);
        }
        else {
          const resetQuantity  = isDocumentTypeDelivery ? item.getTargetQuantity() : item.getSuggestedQuantity();
          item.setQuantity(resetQuantity);
        }
        me.getLoItems().resumeListRefresh(true);
        item.setEdited("0");
        return when.resolve();
      ...
      var quantityToCompare = isDocumentTypeDelivery ? mainItem.getTargetQuantity() : mainItem.getSuggestedQuantity();
      if ((isDocumentTypeDelivery || mainItem.getDeletedFreeItem() === "0") && (
        (modifyPresetQuantityOption === "ODec" && newValue > quantityToCompare) ||
        (modifyPresetQuantityOption === "OInc" && newValue < quantityToCompare) ||
        (modifyPresetQuantityOption === "No" && newValue != oldValue))) {
          var validationMessageId = "QtyEditNotAllowed";
          if (modifyPresetQuantityOption === "ODec") {
            validationMessageId = "QtyEditIncNotAllowed";
          } else if (modifyPresetQuantityOption === "OInc") {
            validationMessageId = "QtyEditDecNotAllowed";
          }
        return MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Attention"), Localization.resolve(validationMessageId), buttonValues).then(function(){
