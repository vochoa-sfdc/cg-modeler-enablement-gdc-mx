 ...
 * @description It is responsible for attaching a debug JSON file as a compressed ZIP to an order's attachments. It ensures that the file is stored with a unique timestamped name and updates
 * or creates an attachment entry accordingly. The function follows an asynchronous flow using Promises.
 * @returns promise
 */
...
if(Utils.isDefined(DebugJSON)) {
  ...
  if (!Utils.isDefined(me.getLoOrderAttachment())) {
    loadLoAttachmentsPromise = BoFactory.loadObjectByParamsAsync(LO_ORDERATTACHMENT, { params:
     [
      {
        "field" : "sdoMainPKey",
        "value" : me.getPKey()
