 ...
 * @namespace CORE
 * @returns promise
 * @description Function is calculating the delivery date.
 *  3 Modes are supported:
 *   - Today: Sets delivery date to today
 *   - No: Sets delivery date to Min Date
 *   - BuiltIn: Is considering LeadTime, Saturday, Sunday and Holiday configuration to calculate delivery date
 */
function presetDeliveryDate(){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
...
switch (me.getBoOrderMeta().getDeliveryDatePolicy()) {
  ...
  case "BuiltIn":
    if (considerLeadTime == "0") {
      ...
      var maxDeliveryDateANSI = Utils.addDays2AnsiDateTime(
        Utils.createAnsiDateTimeToday(),
        me.getBoOrderMeta().getMaxLeadTime()
      );
      var maxDeliveryDate = Utils.convertAnsiDate2Date(maxDeliveryDateANSI);
      ...
      promise = BoFactory.loadObjectByParamsAsync(
        ...
        while (!validDeliveryDateFound) {
          ...
          // calculatedDeliveryDate will be taken as the delivery date,
          // even if it is not a valid working day.
        ...
        return when.resolve(deliveryDate);
      });
    }
    break;
  case "No":
