 ...
 * @returns promise
 * @description  This function is handling  quantity rounding of order item to the unit of measure declared as rounding target.
 *  Following modes are supported:
 *  - Never: No rounding at all
 *  - Up: Round up to rounding unit
 *  - Down: Round down to rounding unit
 *  - Nearest: Round up or down depending what value is closer to current piecest per smalles unit
 *  - UserExit: Invoke a function to define custom rounding behavior
 */
...
if ((isRoundingToggleChanged == "0" && quantityRounding == "RoundingDefaultOff") ||
  ...
  promise = BoFactory.loadObjectByParamsAsync("LoProductQuantityRounding", { "params": 
    [
      { "field": 'ProductPKey', 
        "value": orderItemMain.getPrdMainPKey(), 
        "operator": "EQ" 
      }
    ] 
  })
    .then(function (loProductQuantityRounding) {
    var prdLogisticRoundingItems = loProductQuantityRounding.getItems();
    var currentRoundingRule = "Never";
    var prdLogisticRoundingItem;

    for (var index = 0; index < prdLogisticRoundingItems.length; index++) {
      if ((prdLogisticRoundingItems[index].getRoundingTarget() == 1) &&
