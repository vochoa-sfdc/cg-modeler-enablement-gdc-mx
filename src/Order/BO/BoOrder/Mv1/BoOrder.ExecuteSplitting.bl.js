 ...
 * @namespace CORE
 * @returns promise
 * @description Splits an order into multiple orders if splitting is enabled. 
 *  Creates new order documents for each splitting group, copying relevant data, adjusting attributes, and processing inventories
 *  Cancels the parent order if any splits were created.
 */
function executeSplitting(){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
...
if (me.isSplittingEnabled()) {
  ...
  var createSplittingDoc = function (splittingGroup) {
    ...
    var splittingDoc = BoFactory.instantiate(BO_ORDER, {
      "asn" : me.getAsn(),
    ...
    return me.esSetHeaderAttributes(splittingDoc, splittingGroup)
      ...
      .then(function () {
        ...
        if (Utils.isDefined(wfeInitialStateJson)) {
          splittingDoc.setPhase(BLConstants.Order.PHASE_INITIAL);
