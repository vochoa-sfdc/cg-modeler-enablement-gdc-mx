 ...
 * @description The function is used to get the json query for the product to be added.
 * @this BoOrder
 * @kind TODO_ADD_BUSINESS_OBJECT_TYPE
 * @namespace CORE
 * @returns productForAddQuery
 * @description Function is providing parameters to read product list used to add products.
 */
function getJsonQueryForProductForAdd(){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
...
productForAddParams.push( { "field" : "considerInventory", "value" : me.getBoOrderMeta().getConsiderInventory()});
/** Closed Listing */
...
//Inventory Filter
if (me.getBoOrderMeta().getFilterByCurrentInventory() == '1'){
  productForAddParams.push( { "field" : "filterCurrentInventory", "value" : '1'});
}
...
//sales document assortment filter
if (filterBySdoAssortment == '1') {
  filters.push("SdoAssortment = '1'");
}

//current inventory filter
if(filterByCurrentInventory == '1'){
  filters.push("IsAvailableInventory = '1' ");
}

//adding filters to query
if(filters.length > 0){
  queryCondition += "AND ";
  queryCondition += filters.join(" AND ");
}
