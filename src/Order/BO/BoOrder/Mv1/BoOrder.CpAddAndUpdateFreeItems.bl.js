 ...
 * @description Handles the addition and update of free items in an order. Supports penny-perfect pricing, ensures free items are linked correctly,  
 * and updates order filters and pricing calculations. Implements CGCloud-specific logic for handling polymorphic lookups. 
 * @returns promise
 */
	...
	**/
    ...
    return when.all(deferreds).then(
      function(){
        ...
        var productsUpdated = [];
        me.getLoItems().addItems(itemsToBeAdded);
        ...
        for(var i = 0; i < itemsToBeAdded.length; i++){
          ...
          // Update basket count on item filter
          me.updateItemFilterBasketCount(item, 0, item.quantity);
