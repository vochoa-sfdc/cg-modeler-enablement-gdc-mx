 ...
 * @description Handles before-save logic for BoOrder, managing phase transitions, rewards, pricing logs, and audit trails. It updates order states, assigns responsibilities, and resets special rewards. 
 * Also processes workflow actions and deletes conditions for non-valuated delivery notes if needed.
 * @returns promise
 */
...
if (me.getBoOrderMeta().getGenerateRewardsLog() == "Yes" && me.getBoOrderMeta().getConsiderSelectablePromotion() == "1" && 
...
var userExitPromise = pricingJsonPromise.then(
  function () {
    ...
    if (isOrderPhaseInitial && me.getBoOrderMeta().getConsiderSelectablePromotion() == "1") {
      var deferreds = [];
        ...
        function (loRecentState) {
          ...
          if (me.getDocTaType() === "NonValuatedDeliveryNote") {
            if (Utils.isDefined(me.getLoSdoConditions())) {
              me.getLoSdoConditions().remove();
            } else {
              return BoFactory.loadObjectByParamsAsync(LO_SDOCONDITIONS, {"params":
                [
                  {
                    "field": "sdoMainPKey",
                    "value": me.getPKey(),
