 ...
 * @description This function sets the properties of order by setting the values based on BoOrderMeta
 * @this BoOrder 
...
/*     if (
    ...
    if (
      Utils.isDefined(
        ApplicationContext.get(BLConstants.APPCTX.DSD.RUNNING_TOUR)
      )
    ) {
      me.setTmgMainPKey(
        ApplicationContext.get(BLConstants.APPCTX.DSD.RUNNING_TOUR)
...
.then(function (lookups) {
  ...
  if ( me.getBoOrderMeta().getConsiderBpaDocTaType() == "1" ) {
    if (me.getBoOrderMeta().getSdoSubType() === BLConstants.Order.DOCUMENT_TYPE_ORDER_ENTRY) {
    ...
    else if (me.getBoOrderMeta().getSdoSubType() === BLConstants.Order.DOCUMENT_TYPE_DELIVERY) {
...
.then(function (luSdoMetaDocTATypeSysNumber) {
  if (Utils.isDefined(luSdoMetaDocTATypeSysNumber)) {
    if (
      ...
      var numberGenerationId = me.getBoOrderMeta().getSysNumberPKey();
      if(Utils.isDefined(me.getBoOrderMeta().getInvoiceNumberDefinitionId()) &&  !Utils.isEmptyString(me.getBoOrderMeta().getInvoiceNumberDefinitionId())) {
        numberGenerationId = me.getBoOrderMeta().getInvoiceNumberDefinitionId();
      }
      return SysNumber.getSysNumberAsync(numberGenerationId);
    }
  }
})
