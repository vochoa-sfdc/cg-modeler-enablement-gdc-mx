 ...
 * @description Function creates UOM items for given main items which are not yet available.
 *  It also handles free item special case so that free items are visible in the main list with correct quantities.
 */
...
if(Utils.isDefined(freeItem)) {
  ...
  for(var i=0; Utils.isDefined(missingLiLogisticUnit) && i < missingLiLogisticUnit.length; i++) {
    uoMs.push(me.createNewUoMItem(mainItem, missingLiLogisticUnit[i]));
  }
}
