 ...
 * @param {DomSdoSubType} orderDocumentType
 * @returns promise
 * @description Handles Product Addition using product key , order item template and promotion. If given product (product key + order item template + promotion) is already present in list of items,
 * it does not add newly t only increment the quantity based scanIncrementQuantity for scan mode.
 */
function addItemFromObject(productsForAdd, productPKey, sdoMainPKey, customerPKey, promotionPkey, considerSelectablePromotion, commitDate, itemTemplate, clbMainPKey, criterionAttribute, barcodeScanBehavior, scanIncrementQuantity, mode, uoM, piecesPerSmallestUnit, orderMeta, loSuggestedQuantity, orderDocumentType){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
if(Utils.isDefined(orderMeta) && (orderMeta.getComputePrice() === "4" || orderMeta.getComputePrice() === "5")) {
...
promise = Facade.getObjectAsync(productInformationCP, productInfoJsonQuery)
  ...
  function (lookupData) {
    ...
    var isAddAllowed = !Utils.isDefined(orderDocumentType) || orderDocumentType !== BLConstants.Order.DOCUMENT_TYPE_DELIVERY || itemTemplate.addAllowed === 1;
    
    var filterCountIncrements = [];

    //Check if there exists an item with the same ProductPKey
    if (isAddAllowed && existingItems.length === 0) {
    ...
    if (mode === "addScannedProduct" && barcodeScanBehavior === "SelectIncrease") {
      if (result == "Invalid") {
        ...
        var newError = {
          "level": "error",
          "objectClass" : "BoOrder",
          "messageID" : "NewProductNotAllowed",
          "messageParams" : {
            "orderItemTemplateDescription" : itemTemplate.text,
          }
        };
        messageCollector.add(newError);
        var messages = messageCollector.getMessages().join("<br>");
        var buttonValues = {};
        buttonValues[Localization.resolve("GotIt")] = "ok";
        return MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Attention"), messages, buttonValues);
      }
