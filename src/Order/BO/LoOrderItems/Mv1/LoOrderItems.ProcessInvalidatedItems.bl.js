 ...
 * @returns promise
 * @description This function is checking if the order items are still valid after re-opening an order.
 * It executes checks like :
 *  - is products still valid (Field State, Delivery State ...) 
 *  - are the unit of measures set uo in the correct way
 *  - is it still part of the assortment
 *  - is it still part if the promotion
 *  - ...
 * If the product is invalid quantity is set to 0, the item is no longer editable and a validation message is shown
 */
function processInvalidatedItems(jsonQueryAddProducts, addProductCriterionAttribute, orderItemMetas, hurdleEvaluationHelper){
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
var promise;
//Check for items that are invalidated  (property mergeEngine_invalidated = "1")
...
if (items.length > 0) {
  ...
  promise = promotionPromise.then(function () {
    ...
    var validItems = loProductsForAdd.getItems();
    ...
    var columnName = "Order_Item__c.Id";
    ...
    //sort items according to the text1 so that in invalid item message the items appear in alphabetic order
    var items = me.getAllItems();    var invalidItems = items.filter(function(invalidItems){ return invalidItems.mergeEngine_invalidated == "1";});
    invalidItems.sort((a, b) => {
      if (a.text1 < b.text1) {
        return -1;
      }
      if (a.text1 > b.text1) {
    ...
    //order id is needed to write warning log
    if (invalidItems.length > 0) {
      orderId = invalidItems[0].getSdoMainPKey()
    }
    ...
    for (var i = 0; i < invalidItems.length; i++) {
      ...
      item.setQuantity(0);
      productTextsForMessage += "<br>";
    }
    if (!Utils.isEmptyString(productTextsForMessage)) {

      //Write warning log if there exist invalid items
      AppLog.warn("Invalid Items for order 'ID: " + orderId + "' - " + productTextsForMessage);

      // Display message about invalid removed products
      var buttonValues = {};
      buttonValues[Localization.resolve("OK")] = "ok";
      return MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Attention"), 
                                       Localization.resolve("OrderInvalidatedProductsFound") + '<br>' + productTextsForMessage, buttonValues);
    }
  });
}
else {
