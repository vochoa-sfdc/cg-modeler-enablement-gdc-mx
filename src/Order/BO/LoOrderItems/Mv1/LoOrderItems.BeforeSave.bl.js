 ...
 * @description Removes all items not necessary for save including deleted free items, items with zero quantity but not required to be saved and items with promotion key
 *              having zero quantity.
 */
...
var deletedFreeItems = me.getItemsByParam({"deletedFreeItem": "1"});
for (var j=0; j<deletedFreeItems.length; j++){

  if (deletedFreeItems[j].getObjectStatus()== (STATE.NEW | STATE.DIRTY)){
    deletedFreeItems[j].setObjectStatus(STATE.NEW);
  }
  if (deletedFreeItems[j].getObjectStatus()== (STATE.DIRTY | STATE.PERSISTED)){
...
for (var i=0; i<items.length; i++){
  if ((items[i].getSaveZeroQuantity()== '0' && items[i].getTargetQuantity()== 0 && items[i].getQuantity() == 0)
    ...
    if (items[i].getObjectStatus()== (STATE.NEW | STATE.DIRTY) ){
      items[i].setObjectStatus(STATE.NEW);
    }
    if (items[i].getObjectStatus()==(STATE.DIRTY | STATE.PERSISTED)){
