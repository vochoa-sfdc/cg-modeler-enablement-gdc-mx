 ...
 * @description Function is setting unit of measure information for given (main) item.
 */
...
//if there exist multiple isOrderUnits consider only one for orderUnitInformation
var alreadyConsideredOrderUnit = false;  
...
for (var i = 0; i < unitOfMeasureItems.length; i++) {
  ...
  if (unitOfMeasureItem.getIsOrderUnit() == "1" && !alreadyConsideredOrderUnit) {
    orderUnitInformation += info.unitInformation;
    alreadyConsideredOrderUnit = true;
  }
  else {
