...
//### Load UsrDocMeta
//########################
var promise = BoFactory.loadObjectByParamsAsync(BO_USERDOCMETA, {
  params: [
    {
      field: "pKey",
      value: me.getUsrDocMetaPKey(),
      operator: "EQ"
    }
  ]
}).then(
  function (boUserDocMeta) {
    ...
    return BoFactory.loadObjectByParamsAsync(LU_USER, {
      params: [
        {
          field: "pKey",
  ...
  function (boUserDRAnnotation) {
    ...
    boUserDRAnnotation.setObjectStatus(STATE.NEW | STATE.DIRTY);
    ...
    var callParams = [];
    var callQuery = {};
    var dateFrom = Utils.convertForDBParam(me.getDateFrom(), "DomDate");
  ...
  function (loUserDRRecentState){
    ...
    return BoFactory.loadListAsync(LO_USRTIMEENTRYBYVISITTYPE, {
      params: [
        {
          field: "UsrDailyReportPKey",
          value: me.getPKey(),
          operator: "EQ"
        }
      ]
  ...
  function (loUsrTimeEntryByVisitType) {
    ...
    return BoFactory.loadListAsync(LO_USRTIMEENTRY, {
      params: [
        {
          field: "UsrDailyReportPKey",
          value: me.getPKey(),
...
/*     if(!(Utils.isDefined(ApplicationContext.get('currentTourPKey')) && !Utils.isEmptyString(ApplicationContext.get('currentTourPKey')) && ApplicationContext.get('currentTourStatus') == "Running") && !Utils.isEmptyString(me.getBoUserDocMeta().getWorkUsrTimeEntryMetaPKey())){
  ...
  function (loUsrDRActivity) {
    ...
    loUsrDRActivity.setObjectStatus(STATE.NEW | STATE.DIRTY);
