 ...
 * @description A function to create and save a new time entry record with various attributes and handle related metadata, time calculations, and offsets.
...
if (Utils.isDefined(usrTimeEntryMetaPKey)) {
  promise = BoFactory.loadObjectByParamsAsync("LuTimeEntryType", {
    params: [
      {
        field: "pKey",
        value: usrTimeEntryMetaPKey,
        operator: "EQ"
      }
    ]
  }).then(
...
/*       if (Utils.isDefined(ApplicationContext.get('currentTourPKey')) && !Utils.isEmptyString(ApplicationContext.get('currentTourPKey'))) {
      ...
      if (!Utils.isDefined(me.getLoUsrTimeEntry())) {
        return BoFactory.loadObjectByParamsAsync("LoUsrTimeEntry", {
          params: [
            {
              field: "pKey",
              value: me.getPKey(),
          ...
          function (loUsrTimeEntry) {
            ...
            me.getLoUsrTimeEntry().addItems(items);
