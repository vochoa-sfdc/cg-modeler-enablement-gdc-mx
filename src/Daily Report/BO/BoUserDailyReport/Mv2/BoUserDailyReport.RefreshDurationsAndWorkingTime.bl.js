 ...
 * @returns promise
 * @description This function updates the duration if time entry is present, else creates a new time entry based on configurations.
 */
...
if (Utils.isDefined(currentTimeEntry)) {
  ...
  promise = BoFactory.loadObjectByParamsAsync("LuTimeEntryType", { "params": 
    [
      { "field": 'pKey', 
        "value": currentTimeEntry.getUsrTimeEntryMetaPKey(), 
        "operator": "EQ" 
      }
    ] 
  }).then(
    function (luTimeEntryType) {
      ...
      if (Utils.isDefined(luTimeEntryType) && luTimeEntryType.getShowAggregation() == "1") {
        var listTimeEntryByVisitType = me.getLoUsrTimeEntryByVisitType().getItems();
        ...
        for (var i = 0; i < listTimeEntryByVisitType.length; i++) {
          ...
          if (listTimeEntryByVisitType[i].getPKey() === currentTimeEntry.getUsrTimeEntryMetaPKey() || listTimeEntryByVisitType[i].getText() === currentTimeEntry.getUsrTimeEntryMetaText()){
            ...
            listTimeEntries = me.getLoUsrTimeEntry().getItems();
