...
var promise = Facade.getObjectAsync(BO_CASH, jsonQuery).then(
  ...
  function (loPaymentMeta) {
    ...
    return BoFactory.loadObjectByParamsAsync(BO_WORKFLOW, {
      params: [
        {
          field: "pKey",
  ...
  function (sysReleaseProcessPKey) {   
    if(!Utils.isEmptyString(sysReleaseProcessPKey)){
      return BoFactory.loadListAsync("LoSysReleaseProcessStep", {
        params: [
          {
  ...
  function () {
    ...
    if(me.getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_CASH_CHECK_IN ){
      var jsonQuery = {};
      var jsonParams = [];
