    ...
    if(isTourRunning && (navigation === "StartOfDay")) {
      ...
      if (me.getAllItems().length > 0) {
        ...
        for (i = 0; i < items.length; i++) {
          if (items[i].getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_CASH_CHECK_OUT && items[i].getPhase() === "Initial" && items[i].getUsedForStartOfDay() == "1") {
            visible = true;
            break;
          }
        }
  ...
  case "checkIn":
    if(isTourRunning && (navigation === "EndOfDay")) {
      ...
      if (me.getAllItems().length > 0) {
        items = me.getAllItems();
        for (i = 0; i < items.length; i++) {
          if (items[i].getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_CASH_CHECK_IN  && items[i].getPhase() === "Initial" && items[i].getUsedForEndOfDay() == "1") {
            visible = true;
            break;
          }
        }
