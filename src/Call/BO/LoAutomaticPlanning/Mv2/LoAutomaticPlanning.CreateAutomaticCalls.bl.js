 ...
 * @returns allfinished
 * @description Creates visits out of automatic call planning data/setup
 */
...
var createBoCall = function(arrayVisits) {
  var messageCollector;
  var currentVisitParams = arrayVisits[0];
  var boCall;

  var promise = BoFactory.createObjectAsync("BoCall", currentVisitParams)
  .then(function (object) {

    boCall = object;
    boCall.setObjectStatus(STATE.NEW | STATE.DIRTY);

    //validate newly created visit
  ...
  //remove created visit from array
  arrayVisits.shift();
  return promise;
};
...
for (var i = 0; i < customerCallSettingList.length; i++) {
  ...
  if(validWeek) {
    ...
    visit.params = jsonParams;

    //push visit params to array
    seqence_arguments.push(visit);

    //push function call to array
