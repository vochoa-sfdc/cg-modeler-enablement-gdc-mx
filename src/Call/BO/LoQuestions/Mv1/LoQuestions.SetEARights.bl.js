...
if(isReadOnly){
  // The 'Call::QuestionTabProcess' is calling the setEARights with handing over isReadOnly by usinf the BoCall.isReadOnly
  // This function covers all cases in which call can be locked (e.g. substitution, status, etc) 
  // For these read only calls the relevavant Question fields are locked/ edit rights are removed
  ...
  // This section is reached by calls from filterQuestions.
  // Via this function setEARight is called via isReadOnly=false also for read-only calls just to 
  // set questions visible on POS change
  aclRights = me.getACL();
  aclRights.addRight(AclObjectType.PROPERTY, "done", AclPermission.EDIT);
  aclRights.addRight(AclObjectType.PROPERTY, "value", AclPermission.EDIT);
  aclRights.addRight(AclObjectType.PROPERTY, "questionText", AclPermission.EDIT);
  aclRights.addRight(AclObjectType.PROPERTY, "mandatoryImageId", AclPermission.EDIT);
  me.getItems().forEach(function(question){
    aclRights = question.getACL();
    if(question.getVisible() == "1"){
      if(!aclRights.isVisible(AclObjectType.OBJECT, "LiQuestion")){
