 ...
 * @returns allfinished
 * @description Creates visits out of a trip list
 */
    ...
    for (var idxCustomers = 0; idxCustomers < customerList.length; idxCustomers++) {
      ...
      if ((customer.getValidFrom() <= dateFrom) && (customer.getValidThru() >= dateFrom)) {
        ...
        if(skippedCallsOfCurrentDay === 0){
          ...
          jsonParams.push({
            "field" : "clbStatus",
            "value" : BLConstants.VISIT.STATUS_PLANNED
          });
          jsonParams.push({
            "field" : "dateFrom",
            "value" : dateFrom
...
//Function to create new visits via given args
function createBoCall(arrayVisits) {
