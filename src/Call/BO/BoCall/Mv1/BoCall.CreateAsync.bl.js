...
if (
  ...
  me.setClbStatus(BLConstants.VISIT.STATUS_PLANNED);
  me.setOriginalClbStatus(BLConstants.VISIT.STATUS_PLANNED);
  ...
  promise = BoFactory.loadObjectByParamsAsync(
    ...
    .then(function (customerManagementInfoLookup) {
      ...
      if (
        ...
        ((newParams.isManagedCustomer && newParams.isManagedCustomer.getId() !== "1") ||
...
/*       //check if working in tour context
    ...
    .then(function (lookups) {
      ...
      //StartTime must only be set if captureProceedingTime is enabled and "visit checkin /explicit visit start" is disabled
      if (me.getLuCallMeta().getCaptureProceedingTime() == "1" && me.getLuCallMeta().getIsStartVisitRqrToCheckIn() == "0") {
