 ...
 * @returns promise
 * @description Loading Visit and dependent Object like CustomerManagementInfo, Job Manager and Daily Report
 *  Also handles time capturing for stanard RE visits. 
 *  DSD/Explicit start visit time capturing is handled while Visit start and not in load function.
 */
...
var isDSDMode = newParams && newParams.isCallFromDSD;
...
.then( function (selfJson) {
  ...
  if (me.getLuCallMeta().getCaptureProceedingTime() == "1" &&
    (me.getClbStatus() === BLConstants.VISIT.STATUS_PLANNED || me.getClbStatus() === BLConstants.VISIT.STATUS_INPROGRESS)
  ) {
    //Adding the offset for the following flow => When tour visit is started, user navigates back and then again opens the visit
    //in this flow the offset is not set and start time is added with offset incorrectly in checkStartEndTime.
    //Offset must always be set for editable visits to compare against the offset while visit completion (Timezone which while visit is loaded)
    var currentDateTime = Utils.createDateNow();
    me.setStartTimeEffectiveTimezoneOffset(currentDateTime.getTimezoneOffset());

    //set start time only for "Planned" Visits which do not need an explicit start (Stanard RE visits)
    if (!isDSDMode &&
      me.getLuCallMeta().getIsStartVisitRqrToCheckIn() == "0"
    ) {
  ...
  //### Loading Customer Management Info
...
.then(function (customerManagementInfoLookup) {
  ...
  if (Utils.isDefined(me.getUsrTimeEntryMetaPKey()) && !Utils.isEmptyString(me.getUsrTimeEntryMetaPKey())) {
    ...
    if (Utils.isDefined(ApplicationContext.get('openTimeCardPKey')) && !Utils.isEmptyString(ApplicationContext.get('openTimeCardPKey'))) {
      return BoFactory.loadObjectByParamsAsync("BoUserDailyReport", {
        params: [
          {
            field: "pKey",
            value: ApplicationContext.get('openTimeCardPKey'),
            operator: "EQ"
          }
        ]
      });
