 ...
 * @description This method is responsible for deciding visibility for each question based upon their evaluation result. It receives two inputs:
...
var checkQuestion = function(currentQuestion){
  ...
  var currentPkey = currentQuestion.getJobDefinitionPKey();

  for(var item = 0; item < listToBeEvaluated.length; item++){
    var questionItem = listToBeEvaluated[item];
    var conditionId = questionItem.getCondition();
    if(currentPkey === conditionId){
      currentQuestion.setNestedQuestionIcon("PageStructurePNG");
    }
  }
