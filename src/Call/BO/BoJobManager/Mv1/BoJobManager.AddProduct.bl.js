 ...
 * @async
 * @namespace CORE
 * @description This function is used to add product to given POS based on template settings
...
jDLQuery.params = [
  ...
  {"field" : "prdMainPKey","operator" : "EQ","value" : productPKey},  // Add product filtering
...
if (!Utils.isDefined(liProduct)) {
  ...
  promise = BoFactory.loadObjectByParamsAsync("LuProduct", jsonQuery).then(
    function (lookup) {
      liProduct = {
        "pKey" : PKey.next(),
        "prdMainPKey" : productPKey,
      ...
      return BoFactory.loadObjectByParamsAsync("LuPrdUsageIcon", jsonQuery);
    }).then(
      function (prdImageLookup) {
        liProduct.mediaPath = prdImageLookup.mediaPath;
        liProduct.fileType = prdImageLookup.fileType;
