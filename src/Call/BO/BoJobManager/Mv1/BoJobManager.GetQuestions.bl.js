...
//all the relevant data for questions and survey has already been loaded centrally on call level
var liCurrentPOS = me.getLoPOS().getItemsByParam({"posId": posId})[0];

let faultyJobDefs = me.getLoJobDefinitions().getItemsByParam({
                "dataType": "Toggle",
                "toggleId": " ",
              });
for (let idxFaultyJobDefs = 0; idxFaultyJobDefs < faultyJobDefs.length; idxFaultyJobDefs++) {
...
if (Utils.isDefined(liCurrentPOS) && liCurrentPOS.getQuestionsInitialized()!=="1" && (me.getClbStatus() == BLConstants.VISIT.STATUS_PLANNED || me.getClbStatus() == BLConstants.VISIT.STATUS_INPROGRESS
 || ignoreCallState )) {
