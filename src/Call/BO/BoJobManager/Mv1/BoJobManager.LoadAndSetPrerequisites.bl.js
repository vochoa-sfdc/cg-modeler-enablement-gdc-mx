...
if (bLoadJobDefs && (!Utils.isDefined(me.getLoJobDefinitions()) || loadType === "Remagnetization")) {
  ...
  deferreds.push(BoFactory.loadListAsync("LoJobDefinitions", jsonQuery).then(
    function (list) {
  ...
  /* ---------------------------------------------------------------- */
  deferreds.push(BoFactory.loadListAsync("LoMagnetizedJobList", jsonQuery).then(
    function (list) {
...
if (bJobRelevant && !Utils.isDefined(me.getLoExistingJobListJobs())) {
  deferreds.push(BoFactory.loadListAsync("LoJobListJobs", jsonQuery).then(
    function (list) {
...
var promise = when.all(deferreds).then(
  function () {
    ...
    if (magnetizationUpdateRequired) {
      ...
      if (me.getOriginalClbStatus() === "Planned") {
        ...
        if (NonStandardJobList.length > 0) {
          var bMagnetizeJobLists = (me.getClbStatus() === BLConstants.VISIT.STATUS_PLANNED || 
                                    me.getClbStatus() === BLConstants.VISIT.STATUS_INPROGRESS || 
                                    me.getClbStatus() === BLConstants.VISIT.STATUS_COMPLETED);
          var lastJobListPKey = "";
    ...
    if(bLoadQuestions && !Utils.isDefined(me.getLoQuestions())){
      ...
      deferreds2.push(BoFactory.loadListAsync("LoQuestions", jsonQuery).then(
        function (list) {
          list.setObjectStatus(STATE.PERSISTED);
          ...
          me.get('loQuestions').addItemChangedEventListener(me.onQuestionChanged, me, 'loQuestions');
        }
      ));
    }
    ...
    if (bLoadSurveys && (sLoadType === "StatusChange_Delete" || sLoadType === "StatusChange_Complete" || sLoadType === "ExceptionCard")) {
      ...
      deferreds2.push(BoFactory.loadListAsync("LoSurveys", jsonQuery).then(
        function (list) {
