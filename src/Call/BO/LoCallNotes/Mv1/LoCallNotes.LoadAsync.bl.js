...
var promise = Facade.getListAsync(LO_CALLNOTES, jsonQuery).then(
  function (notes) {
    ...
    //LoNotes is loaded after setEARights of BoCall is called
    //means if BoCall is read-only also the notes are read only.
    //For visits with status "Planned" notes should always be editable.
    boCall.setEARightsForNotes();
