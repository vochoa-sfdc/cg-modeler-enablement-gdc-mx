 ...
 * @returns me
 * @description this function retrieves, sorts, and formats the list of notes associated with a Visit for the Visit Card.
 */
...
if(currentNoteExists) {
  ...
  numberVisibleCalls += 1;
}
else {
  me.cardItemCount = notesList.length;
}
notesList = notesList.splice(0, numberVisibleCalls);
...
for(i=0;i<notesList.length;i++) {
  ...
  if(ApplicationContext.get('user').getPKey() == notesList[i].responsiblePKey) {
    notesList[i].noteSubText = notesList[i].noteSubText + " - " + Localization.resolve("CardVisitNotes_You");
  }
  else {
