...
var promise = dateDefer.then(
  function() {
    ...
    var statuses = `${BLConstants.VISIT.STATUS_PLANNED},${BLConstants.VISIT.STATUS_INPROGRESS}`;
    if (filterCalls !== "All") {
      jsonParams.push({
        "field" : "ClbStatus",
        "operator" : "EQ",
        "value" : (filterCalls == "Open") ? statuses : filterCalls
      });
    }
  ...
  me.forEach(function(call) {
    if (!Utils.isEmptyString(call.getBpaMainPKey()) && (call.getClbStatus() === BLConstants.VISIT.STATUS_PLANNED || call.getClbStatus() === BLConstants.VISIT.STATUS_INPROGRESS)
       && call.getCompanyRequired() !== "Not required" && call.getClbMetaType() !== "Phone") {
      if (markerIdx > 23) {
