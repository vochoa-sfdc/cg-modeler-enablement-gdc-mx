 ...
 * @returns promise
 * @description this function calculates the visit completion rate for a card based on calls filtered by date and status.
 */
...
.then(function () {
  ...
  if(items.length > 0){
    ...
    items.forEach(function(item){
      ...
      if(item.getClbStatus() === "InProgress" && !Utils.isDefined(inProgressVisitRow)){
        inProgressVisitRow = item;
      }
    });
    ...
    var inProgressVisits = 0;
    if(Utils.isDefined(inProgressVisitRow)){
      inProgressVisits = inProgressVisitRow.clbStatusCount;
    }

    var numberOfRelevantCalls = completedVisits + plannedVisits + canceledVisits + inProgressVisits;
