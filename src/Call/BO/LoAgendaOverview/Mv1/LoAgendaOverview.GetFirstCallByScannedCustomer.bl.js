 ...
 * @description Returns the first callPKey matching the filters - bpaMainPKey, dateFrom and clbStatus
 * @returns promise
 */
...
else
{
  ...
  if(me.getAllItems().length > 0)
  {
    ...
    var dToday = Utils.createAnsiDateToday();
    me.setFilterArray([ {"bpaMainPKey" : customerPKey, "op" : "EQ"}, {"dateFrom" : dToday, "op" : "EQ"}, {"clbStatus" : BLConstants.VISIT.STATUS_PLANNED, "op": "EQ"} ]);
...
if(me.getAllItems().length > 0)
{
  ...
  if (me.getItems().length > 0) {
    callPKey = me.getItems()[0].getPKey();
  }
