 ...
 * @async
 * @namespace CORE
 * @param {String} includeCurrentUser - "1" to include, "0" to exclude
 * @returns promise
 * @description Loads team members for current user (supervisor) and optionally includes current user
 */
  ...
  function (loTeamOfSupervisor) {
    ...
    if (includeCurrentUser == "1") {
      if (Utils.isEmptyString(listTeamOfSupervisor)) {
        ...
        if (!Utils.isEmptyString(me.getPKey()) && !listTeamOfSupervisor.includes(me.getPKey())) {
          listTeamOfSupervisor += ", '" + me.getPKey() + "'";
        }
      }
    }
