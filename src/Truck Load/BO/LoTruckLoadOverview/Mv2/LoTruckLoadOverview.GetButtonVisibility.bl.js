...
if (Utils.isDefined(ApplicationContext.get('currentTourStatus')) && ApplicationContext.get('currentTourStatus').toLowerCase() !== "running") {
  ...
  switch (navigationMode) {
    ...
    case "StartOfDay":
      if (mode === "ProductCheckOut") {
        if (!Utils.isEmptyString(runningTour)) {
          ...
          if (me.getAllItems().length > 0) {
            ...
            for (i = 0; i < items.length; i++) {
              if (items[i].getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_PRODUCT_CHECK_OUT && items[i].getPhase() === "Initial" && items[i].getUsedForStartOfDay() == "1") {
                visible = true;
                break;
              }
            }
      ...
      //##################
    case "EndOfDay":
      if (!Utils.isEmptyString(runningTour) &&  mode===BLConstants.Order.DOCUMENT_TYPE_PRODUCT_CHECK_IN) {
        if (!currentOpenTourIsRunningTour) {
          visible = false;
          break;
        }
        ...
        if (me.getAllItems().length > 0) {
          items = me.getAllItems();
          for (i = 0; i < items.length; i++) {
            if (items[i].getDocumentType() === BLConstants.Order.DOCUMENT_TYPE_PRODUCT_CHECK_IN && items[i].getPhase() === "Initial" && items[i].getUsedForEndOfDay() == "1") {
              visible = true;
              break;
            }
          }
