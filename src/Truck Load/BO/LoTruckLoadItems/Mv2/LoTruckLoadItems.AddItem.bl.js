...
var promise = Facade.getObjectAsync("LuProductUom", jsonQuery_productInformation).then(
  function (lookupData) {
    ...
    return BoFactory.loadObjectByParamsAsync("LuProduct", {
      params: [
        {
          field: "pKey",
          value: ProductPKey,
  ...
  function (lookupData) {
    ...
    if (existingItems.length === 0) {
      if (itemTemplate.getAddAllowed() === '0' || (phase === "Released" || phase === "Canceled")) {
        ...
        return MessageBox.displayMessage(Localization.resolve("MessageBox_Title_Warning"), Localization.resolve("AddNotAllowedMessage"), buttonValues).then(
          function () {
            return result;
          }
        );
        ...
        return me.addMissingUoMsToItem(li, itemTemplate, "").then(
          function () {
            ...
            if (documentType === BLConstants.Order.DOCUMENT_TYPE_PRODUCT_CHECK_OUT) {
